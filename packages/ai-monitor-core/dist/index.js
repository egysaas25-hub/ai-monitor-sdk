...
// ../node_modules/.pnpm/axios@1.13.2/node_modules/axios/dist/node/axios.cjs
var require_axios = __commonJS({
  "../node_modules/.pnpm/axios@1.13.2/node_modules/axios/dist/node/axios.cjs"(exports2, module2) {
    "use strict";
...
5. Confidence score (0-1)`;
      ...
      buildLogsAnalysisPrompt(logs) {
        const logSummary = logs.slice(0, 20).map(
          (l) => `[${l.timestamp.toISOString()}] ${l.level}: ${l.message}`
        ).join("\n");
...
5. Suggested fixes`;
      ...
      buildMetricsAnalysisPrompt(metrics) {
        const metricSummary = metrics.map(
          (m) => `${m.name}: ${m.value}${m.unit || ""} at ${m.timestamp.toISOString()}`
        ).join("\n");
...
var AIMonitor = class {
  // IAIService type from ai-service.ts
  constructor(config = {}) {
  ...
  async start() {
    ...
    return new Promise((resolve) => {
      this.server.listen(this.config.port, this.config.host, () => {
        ...
        if (this.config.enabled) {
          this.logger.info(
            `\u{1F680} AI Monitor started on http://${this.config.host}:${this.config.port}`
          );
          ...
          this.logger.info(
            `\u{1F680} AI Monitor started on http://${this.config.host}:${this.config.port} (DISABLED MODE)`
          );
...
${analysis.suggestions.map((s) => `\u2022 ${s}`).join("\n")}` : ""}`,
        ...
        this.logger.info(`\u{1F916} AI Enhanced: [${alert.severity}] ${alert.title} (Confidence: ${Math.round((analysis.confidence || 0) * 100)}%)`);
      } catch (error) {
