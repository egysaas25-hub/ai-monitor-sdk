...
var SEVERITY_EMOJI = {
  CRITICAL: "\xF0\u0178\u0161\xA8",
  WARNING: "\xE2\u0161\xA0\xEF\xB8\x8F",
  INFO: "\xE2\u201E\xB9\xEF\xB8\x8F"
};
var DiscordNotifier = class {
  ...
  async sendAlert(alert) {
    const emoji = SEVERITY_EMOJI[alert.severity] || "\xF0\u0178\u201C\xA2";
...
\`\`\``,
  ...
  async sendPipelineStatus(status) {
    const statusEmoji = status.status === "SUCCESS" ? "\xE2\u0153\u2026" : status.status === "FAILURE" ? "\xE2\x9D\u0152" : "\xE2\u0161\xA0\xEF\xB8\x8F";
    ...
    if (status.changes && status.changes.length > 0) {
      fields.push({ name: "Changes", value: status.changes.map((c) => `\xE2\u20AC\xA2 ${c}`).join("\n"), inline: false });
    }
    await this.post({
      embeds: [
        {
  ...
  async sendDeploymentNotification(deployment) {
    const emoji = deployment.status === "SUCCESS" ? "\xF0\u0178\u0161\u20AC" : "\xE2\x9D\u0152";
    ...
    if (deployment.changes && deployment.changes.length > 0) {
      fields.push({ name: "Changes", value: deployment.changes.map((c) => `\xE2\u20AC\xA2 ${c}`).join("\n"), inline: false });
    }
    await this.post({
      embeds: [
        {
  ...
  async sendDailyReport(report) {
    await this.post({
      embeds: [
        {
          title: "\xF0\u0178\u201C\u0160 Daily Health Report",
          color: 5793266,
          fields: [
            ...
            { name: "Top Issues", value: report.topIssues.map((i) => `\xE2\u20AC\xA2 ${i}`).join("\n") || "None", inline: false }
          ],
...
Changes:
${status.changes.map((c) => `\xE2\u20AC\xA2 ${c}`).join("\n")}` : "";
    return `
...
${deployment.url ? `URL: ${deployment.url}` : ""}
  ...
  generateReportHtml(report) {
    const overallStatus = report.totalAlerts === 0 ? "\xE2\u0153\u2026 Healthy" : "\xE2\u0161\xA0\xEF\xB8\x8F Issues Found";
    ...
    return `
      <div style="font-family: Arial, sans-serif; max-width: 600px;">
        <div style="background: #007bff; color: white; padding: 20px; border-radius: 5px 5px 0 0;">
          <h1 style="margin: 0;">\xF0\u0178\u201C\u0160 Daily Health Report</h1>
...
${report.topIssues.map((issue, i) => `${i + 1}. ${issue}`).join("\n")}` : "";
    return `
\xF0\u0178\u201C\u0160 Daily Health Report - ${report.date.toLocaleDateString()}

Overall Status: ${overallStatus}

Summary:
\xE2\u20AC\xA2 Total Alerts: ${report.totalAlerts}
\xE2\u20AC\xA2 Critical: ${report.criticalAlerts}
\xE2\u20AC\xA2 Auto-Fixed: ${report.autoFixes}
\xE2\u20AC\xA2 Uptime: ${report.uptime}${topIssuesStr}
    `.trim();
  }
...
var SlackNotifier = class {
  ...
  async sendPipelineStatus(status) {
    ...
    if (status.changes && status.changes.length > 0) {
      fields.push({
        title: "Changes",
        value: status.changes.map((c) => `\xE2\u20AC\xA2 ${c}`).join("\n"),
        short: false
      });
    }
    const attachment = {
  ...
  async sendDeploymentNotification(deployment) {
    if (!this.enabled) return;
    const emoji = deployment.status === "SUCCESS" ? "\xE2\u0153\u2026" : "\xE2\x9D\u0152";
    ...
    if (deployment.changes && deployment.changes.length > 0) {
      fields.push({
        title: "Changes",
        value: deployment.changes.map((c) => `\xE2\u20AC\xA2 ${c}`).join("\n"),
        short: false
      });
    }
    const attachment = {
  ...
  async sendDailyReport(report) {
    ...
    await this.axios.post(this.webhookUrl, {
      attachments: [
        {
          color,
          title: `\xF0\u0178\u201C\u0160 Daily Health Report - ${report.date.toLocaleDateString()}`,
  ...
  getSeverityEmoji(severity) {
    switch (severity) {
      case "CRITICAL":
        return "\xF0\u0178\u0161\xA8";
      case "WARNING":
        return "\xE2\u0161\xA0\xEF\xB8\x8F";
      case "INFO":
        return "\xE2\u201E\xB9\xEF\xB8\x8F";
      default:
        return "\xF0\u0178\u201C\xA2";
    }
  }
  ...
  getStatusEmoji(status) {
    switch (status) {
      ...
      case "FAILURE":
        return "\xE2\x9D\u0152";
      case "ABORTED":
        return "\xE2\x8F\xB9\xEF\xB8\x8F";
      case "UNSTABLE":
        return "\xE2\u0161\xA0\xEF\xB8\x8F";
      default:
        return "\xF0\u0178\u201C\u2039";
...
*Changes:*
${deployment.changes.map((c) => `\xE2\u20AC\xA2 ${c}`).join("\n")}` : "";
...
*Overall Status:* ${report.totalAlerts === 0 ? "\xE2\u0153\u2026 Healthy" : "\xE2\u0161\xA0\xEF\xB8\x8F Issues Found"}

\xF0\u0178\u201C\u02C6 *Summary:*
\xE2\u20AC\xA2 Total Alerts: ${report.totalAlerts}
\xE2\u20AC\xA2 Critical: ${report.criticalAlerts}
\xE2\u20AC\xA2 Auto-Fixed: ${report.autoFixes}
\xE2\u20AC\xA2 Uptime: ${report.uptime}
